...
# kaldi
KALDI_PATH=${HOME_DIR}/software/kaldi # using torchaudio instead
...
if [ $stage -le 0 ]; then
    for dset in ${MODE}; do
        ...
        ./partialspoof/utils/flac_to_wav.sh $FLAC_SCP $WAV_PATH ${SV56_PATH} 
        
	# 3. Data Preparation
	# 1) wav.scp
        awk -vpath=${WAV_PATH} '{print $1" "path$1".wav"}' ${FLAC_SCP} \
		> ${SOURCEDATA_DIR}/${dset}/wav.scp
	## Alternative way to prepare wav.scp: -> string is too long...
        #find ${WAV_PATH}/*.wav > ${SOURCEDATA_DIR}/${dset}/wav.list 
        #cat ${SOURCEDATA_DIR}/${dset}/wav.list | xargs -i basename {} .wav |\
	#     paste -d' ' - data/${dset}/wav.list > data/${dset}/wav.scp 
        #rm -rf data/${dset}/wav.list
	#
	# 2) reco2dur
        # wav-to-duration ark:${SOURCEDATA_DIR}/${dset}/wav.scp ark,t:${SOURCEDATA_DIR}/${dset}/reco2dur	
        python ./tools/wav2dur.py \
		${SOURCEDATA_DIR}/${dset}/wav.scp \
		${SOURCEDATA_DIR}/${dset}/reco2dur	

	# 3) Prepare reco2type with format for each line as <recid> <type>
        # Modified the 5th column: Replace - as bonafide for bonafide samples
        PROTOCOL=${SOURCEDATA_DIR}/protocols/ASVspoof2019.LA.cm.${dset}.trn.txt
        awk '{if($5=="bonafide"){print $2" bonafide"}
	      else print $2" "$4}' ${PROTOCOL} > ${SOURCEDATA_DIR}/${dset}/reco2cls
...
# Configuration
MIN_SIL_MS=30     # Treat nonspeech part as speech if it is shorter than
MIN_SEG_MS=30     # Treat a speech segment as nonspeech if it is shorter than 
SHIFT_STEP_MS=10 # 
...
if [ $stage -le 1 ]; then
	...
	LABEL2NUM=${SOURCEDATA_DIR}/label/label2num_mul
        ...
        # 1. Calculating VAD, Currently we support kaldi_based VAD, max_energy_based VAD, pyannote
	# You can either precompute VAD, or compute them when voting.
	# 1) Extract VAD
	#    i) 
	#    window shift log-energy-based VAD from kaldi
	#    ii) pyannote:
#        local/vad/vad_pyannote.sh \
#		--DATA_PATH ${SOURCEDATA_DIR}/${dset} \
#		--MODEL_NAME sad_ami
	...
	# multi can implement kaldi, rms, pyannote
        # output: vad_dict.npy
#	python partialspoof/utils/cal_vad_vote.py \
...
#	      	#--pyannote_pretrain_vad ${SOURCEDATA_DIR}/${dset}/vad/pyannote_sad_dihard.npy || exit 1
	...
	# WAV2TYPE is used when you want to use specific labels other than 1 for speech segments
	# like 2 for A01, .... 
        python  ./local/get_standard_vad.py --TYPE "zlnpy"\
                                           --VAD_FILE data/${dset}/vad/vad_dict.npy \
                                           --VAD_SAVE_DIR ${STANDARDVAD_DIR}/utt-vad \
                                           --LABEL2NUM ${LABEL2NUM}  \
                                           --WAV2TYPE ${SOURCEDATA_DIR}/${dset}/reco2cls \
					   --RECO2DUR ${SOURCEDATA_DIR}/${dset}/reco2dur \
	                                   --shift_step_ms ${SHIFT_STEP_MS} \
					   --min_sil_ms ${MIN_SIL_MS} --min_seg_ms ${MIN_SEG_MS}  || exit 1
	# TODO from here
	# 0 for nonspeech, 1 for speech
	# 0 for non, 1 for bona, 2 for spoof
	# 0 for non, 1 for bona, >2 for different spoofing methods
	# 1 for bona, [0-9][2-9] for different spoofing methods, >100 for nonspeech
#
